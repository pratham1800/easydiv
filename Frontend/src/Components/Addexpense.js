...
function Addexpense({ setModal, modal }) {
  ...
  const getSum = () => {
    ...
    Object.keys(listobj).map((f) => {
      if (payer === f) {
        console.log("Cost",parseInt(listobj[f]));
  ...
  const saveexpense = async () => {
    ...
    if (t > cost) {
      ...
      var uowed = 0;
      if (user._id === payer) {
        exp = listobj[user._id];
        // uowed = Math.abs(listobj[user._id].toFixed(2));
      } else {
        exp = listobj[user._id];
        // uowe = Math.abs(listobj[user._id]).toFixed(2);
      }
      ...
      let useexp = [ ...user.expenses ];
      console.log("Initial user exp: ",useexp);
      if (payer === user._id) {
        Object.keys(listobj).map((j) => {
          let f = useexp.find((exp,i) => {
            if(exp.user._id === j){
              return i
            }
          })
          ...
          if (f){
            let temp = useexp[f]
            temp.amount = parseInt(temp.amount || 0) + parseInt(listobj[j])
            useexp[f] = temp
          }
          else {
            let temp = {}
            temp.amount = 0 + parseInt(listobj[j])
            temp.user={
              _id: j,
              name: ""
            }
        ...
        Object.keys(listobj).map((j) => {
          ...
          if (j === payer) {
            if (useexp[f])
              useexp[f].amount =
                parseInt(useexp[f].amount) + parseInt(listobj[f] / tags.length);
            else useexp[f].amount = 0 + parseInt(listobj[f] / tags.length);
          } else if (f === user._id) {
            if (useexp[f])
              useexp[f].amount = parseInt(useexp[f].amount) + parseInt(listobj[f]);
            else useexp[f].amount = 0 + parseInt(listobj[f]);
          }
        });
      }
      console.log(useexp);
      Object.keys(useexp).map((f) => {
        console.log("User Exp:",f);
        if (useexp[f].user._id != user._id) {
          if (useexp[f].amount > 0) {
            uowe = uowe + parseInt(useexp[f].amount);
            console.log("owe", uowe);
          } else if (useexp[f].amount < 0) {
            uowed = uowed + Math.abs(parseInt(useexp[f].amount));
